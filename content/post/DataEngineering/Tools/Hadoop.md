...
## Hadoop 기업 사례
### 야후
야후는 2000년대 초반부터 대규모 웹 크롤링 및 인덱싱 작업을 수행하면서 방대한 양의 데이터를 수집하고 있었다. 기존의 데이터 처리 인프라로는 수집된 데이터를 효율적으로 저장하고 처리하는 데에 한계가 있었고, 처리 능력이 부족해졌다.

이를 해결하고자 하둡을 채택했다. HDFS를 통해 방대한 데이터를 효율적으로 저장하고, 맵리듀스를 사용하여 대규모 데이터 세트를 병렬로 처리하여 데이터 처리 속도를 크게 향상시켰다.
