...
따라서, 목표 비용을 위하여 RDBMS보다는 Bigquery를 선택하는 것이 필요하기 때문에  제가 선택한 방법은 2번 데이터 복제(with BigQueyr) 방법입니다.
그리고 쿼리 속도를 줄이기 위해서는 쿼리의 복잡성을 최소화는 것이 중요합니다. 따라서, 요구사항에 맞춰 테이블을 설정하고 오직 SELECT를 통해 대시보드를 표현해야 합니다. 더 구체적인 아키텍처는 다음과 같습니다.
