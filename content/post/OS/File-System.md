...
## NT 파일 시스템 (NTFS)
NTFS는 FAT32의 단점을 보완하여 만들어진 방식이다.

## ext 파일 시스템
ext 파일 시스템은 리눅스 운영 체제를 위해 설계된 파일 시스템 구조로, 유닉스 파일 시스템의 철학과 구조를 따르면서도 리눅스 특정 요구에 맞게 최적화된 시스템이다.

ext 파일 시스템은 여러 버전이 있으며, 주요 버전은 ext, ext2, ext3, ext4가 있는데 저널링 기능이 생긴 ext3부터 알아보고자 한다.

### ext3 파일 시스템
ext3 파일 시스템은 유닉스 파일 시스템과 마찬가지로 inode를 사용하여 파일을 저장하고 관리하는 방식이다.
ext3 파일 시스템은 저널링 기능을 가진 파일 시스템이다. 저널링 기능이란, 파일 시스템의 변경 사항을 추적하기 위해 만들어진 기능으로, 삭제된 파일을 복구하는데 사용하기도 한다.

ext3는 세 가지 저널링 모드를 지원한다.
- Writeback: 메타데이터만 저널링하고, 데이터는 즉시 디스크에 기록하지 않는다.
- Ordered: 메타데이터는 저널링하고, 데이터는 메타데이터가 기록되기 전에 디스크에 기록한디. 이는 데이터 무결성을 보장하면서 성능도 어느 정도 유지한다.
- Journal: 데이터와 메타데이터 모두를 저널링한다. 가장 안전한 방식이지만 성능에 영향을 미친다.
